<?php
   ...
   * utilizing eAccelerator extension (http://eaccelerator.net/)<br>
   *
...
//    $eaccelerator_id  = 'smarty|'.$cache_id.'_'.$tpl_file;
    $eaccelerator_id  = 'smarty|'.$cache_id;

    switch ($action)
      {
      case 'read':
        // read cache from shared memory
        $cache_content = @gzuncompress(eaccelerator_get($eaccelerator_id));
//        print_r($eaccelerator_id);
        if (!is_null($cache_content) && _eaccelerator_hasexpired($cache_content))
          ...
          $cache_content = null;
          //eaccelerator_rm($eaccelerator_id);
          }
      ...
      case 'write':
        ...
        //Ложим инфу в eaccelerator
        $return = eaccelerator_put($eaccelerator_id, gzcompress($cache_content,8), $ttl);
        break;
      ...
      case 'clear':
        ...
        //global $dbg_sql_queries; $dbg_sql_queries[] = array('sql_query'=>$sql); sysDBQuery($sql);
        //if (mysql_errno()!=0) sysException ('smarty_cache_eaccelerator', DATABASE_ERROR, mysql_error()."<br> $sql");
        ...
        if (mysql_errno()!=0) sysException ('smarty_cache_eaccelerator', DATABASE_ERROR, mysql_error()."<br> $sql");
        //Удаляем ключи из eacceleratora
        $eaccelerator_keys = array();
        while ($row = mysql_fetch_assoc($mysql_result))
          {
          $eaccelerator_id  = 'smarty|'.$row['cache_id'];
          eaccelerator_rm($eaccelerator_id);
          }
        mysql_free_result($mysql_result);

        //Удаляем из mysql ключи удаленные из eacceleratora
