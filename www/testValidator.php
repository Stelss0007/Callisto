...
class validateForm
  ...
  function validateForm($input = array()) // Yes, post the the whole $_POST/$_GET array into the function
    ...
    // Config
    $this->error_wrapper['start'] = "";
    $this->error_wrapper['end'] = "";
  ...
  function allErrors($masks = array())
    ...
    foreach ($this->error as $key => $value)
      ...
      echo $this->error_wrapper['start']."".ucfirst($key).": ".trim($value).$this->error_wrapper['end'];
      }
    }
...
echo '2012-1-1 = '.strtotime('2012-1-1')." date = ".date('Y-m-d', 1325368800);
