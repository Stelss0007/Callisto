...
//    print_r(debug_backtrace());exit;
    ...
    public function setError($error_message = 'User Error')
    {
        $callee = debug_backtrace();
        $callee = $callee[0];
        $error_array = ['message' => $error_message, 'file' => $callee['file'], 'line' => $callee['line']];
...
//        ]);
    ...
    public function exeptionCatcher($exception)
    {
    ...
    public function showErrors()
    {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <b>
                                            <?php
                                            if ($traceValue['class']) {
                                                echo $traceValue['class'];
                                            }
                                            if ($traceValue['type']) {
                                                echo $traceValue['type'];
                                            }
                                            if ($traceValue['function']) {
                                                echo $traceValue['function'] . '(<span class="params">';
                                            }
                                            ...
                                            if ($traceValue['args']) {
                                                ...
                                                foreach ($traceValue['args'] as $key2 => $arg) {
                                                    if (is_string($arg)) {
                                                        ...
                                                        echo '<span class="arg-detail" onClick="showArgument(' . $key . ',' . $key2 . ');">' . get_class(
                                                                $arg
                                                            ) . '</span>';
                                                        ...
                                                        echo '<span class="arg-detail" onClick="showArgument(' . $key . ',' . $key2 . ');">Array(' . sizeof(
                                                                $arg
                                                            ) . ')</span>';
                                                        echo '<div class="hidden" id="param_detail_' . $key . '_' . $key2 . '"><pre>';
                                                        var_dump($arg);
        ...
        <div class="global-vars">
            ...
            <pre><span class="global-var">$_SERVER</span> = <?php print_r($_SERVER) ?>
                <pre>
          <pre><span class="global-var">$_COOKIE</span> = <?php print_r($_COOKIE) ?>
              <pre>
        </div>
        </body>
        <?php
    ...
    public function showWarnings()
    {
        ...
        foreach ($this->warning_array as $verror) {
            $debug->warnings[] = "PHP WARNING:" . $verror['message'] . ' (' . str_replace(
                    '\\',
                    "/",
                    $verror['file']
                ) . ' in line ' . $verror['line'] . ')';
        ...
        foreach ($this->notice_array as $verror) {
            $debug->notices[] = "PHP NOTICE:" . $verror['message'] . ' (' . str_replace(
                    '\\',
                    "/",
                    $verror['file']
                ) . ' in line ' . $verror['line'] . ')';
