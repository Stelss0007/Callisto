...
function appInit()
  ...
  // ????????? ???????? PHP
  if (\App::$config['locale.lc_all']) setlocale(LC_ALL, \App::$config['locale.lc_all']);
	  else setlocale(LC_ALL, 'ru_RU.CP1251');
  ignore_user_abort(true);

  // ???? ????? ?????????? ?????? ? ????? ?? ???????
  if (\App::$config['DB.Encoded'])
    {
    \App::$config['DB.UserName'] = base64_decode(\App::$config['DB.UserName']);
    \App::$config['DB.Password'] = base64_decode(\App::$config['DB.Password']);
    \App::$config['DB.Encoded'] = 0;
    }
...
function appVarSetCached($component, $cacheKey, $value=null, $ttl=null)
  ...
  elseif (\App::$config['Var.caching'] == 'xcache')
    {
		if (!$ttl) $ttl = \App::$config['Var.cache_lifetime'];
...
function appVarGetCached($component, $cacheKey)
  ...
  if (isset(\App::$config[$cacheKey])) return \App::$config['appVar_cache'][$cacheKey];
  ...
  elseif (\App::$config['Var.caching'] == 'apc')
    {
		...
		if ($success) \App::$config['appVar_cache'][$cacheKey] = $apc_value;
    }

  if (isset(\App::$config['appVar_cache'][$cacheKey])) return \App::$config['appVar_cache'][$cacheKey];
    else return;
  }
