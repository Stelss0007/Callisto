...
class Debuger
{
    ...
    public function debug($name, $var = null, $type = LOG)
    {
        ...
        if (!empty($var)) {
            if (is_object($var) || is_array($var)) {
                ...
                echo 'var val' . preg_replace('~[^A-Z|0-9]~i', "_", $name) . ' = eval("(" + object' . preg_replace(
                        '~[^A-Z|0-9]~i',
                        "_",
                        $name
                    ) . ' + ")" );' . NL;
    ...
    public function debugAdd($name, $var = null, $type = LOG)
    {
        if (empty(\App::$config['debug.enabled'])) {
            return;
        }
        ...
        if (!empty($var)) {
            if (is_object($var) || is_array($var)) {
                $object = json_encode($var);
                $this->debug_private_messages .= 'var object' . preg_replace(
                        '~[^A-Z|0-9]~i',
                        "_",
                        $name
                    ) . ' = \'' . str_replace("'", "\'", $object) . '\';' . NL;
                $this->debug_private_messages .= 'var val' . preg_replace(
                        '~[^A-Z|0-9]~i',
                        "_",
                        $name
                    ) . ' = eval("(" + object' . preg_replace('~[^A-Z|0-9]~i', "_", $name) . ' + ")" );' . NL;
                switch ($type) {
                    case LOG:
                        $this->debug_private_messages .= 'console.debug(val' . preg_replace(
                                '~[^A-Z|0-9]~i',
                                "_",
                                $name
                            ) . ');' . NL;
                        break;
                    case INFO:
                        $this->debug_private_messages .= 'console.info(val' . preg_replace(
                                '~[^A-Z|0-9]~i',
                                "_",
                                $name
                            ) . ');' . NL;
                        break;
                    case WARN:
                        $this->debug_private_messages .= 'console.warn(val' . preg_replace(
                                '~[^A-Z|0-9]~i',
                                "_",
                                $name
                            ) . ');' . NL;
                        break;
                    case ERROR:
                        $this->debug_private_messages .= 'console.error(val' . preg_replace(
                                '~[^A-Z|0-9]~i',
                                "_",
                                $name
                            ) . ');' . NL;
    ...
    public function startRenderPage()
    {
        if (appIsAjax()) {
            return;
        }
