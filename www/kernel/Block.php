...
class Block extends AppObject
{
    ...
    final public function tplFileName($method, $debug = false)
    {
        ...
        if (file_exists(
            $this->root_dir . 'themes/' . $this->theme . '/blocks/' . $this->name . '/' . $method . '.tpl'
        )) {
...
//    if(!$this->smarty->is_cached($tpl_dir, $ObjectName))
    ...
    function loadBlockLang($blockName)
    {
        ...
        } elseif (($this->lang != $this->lang_default) && file_exists(
                "blocks/$blockName/lang/$this->lang_default/lang.conf"
            )
        ) {
